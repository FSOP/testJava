...
public class DbInsertSATCAT extends DbBottom {
    ...
    public void insertSATCAT(BufferedReader br){
        ...
        String line;
        try {
            state.executeQuery("delete from satcat");
            PreparedStatement pstmt = conn.prepareStatement("insert into satcat values (?, ?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
            while ((line = br.readLine()) != null) {

                pstmt.setString(1, line.substring(0, 11));   //Int' Designator
                pstmt.setString(2, line.substring(13, 18));   //Norad
                pstmt.setString(3, line.substring(19, 20));   //Multiple NameFlag
                pstmt.setString(4, line.substring(20, 21));   //Payload Flag
                pstmt.setString(5, line.substring(21, 22));    //Operational Status Code
                pstmt.setString(6, line.substring(23, 47));   //Satellite Names
                pstmt.setString(7, line.substring(49, 54));   //Ownership
                ...
                pstmt.setString(11, line.substring(87, 94));  //Orbital Period
                pstmt.setString(12, line.substring(96, 101)); //Inclination
